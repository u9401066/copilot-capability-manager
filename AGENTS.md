...
| `/cp.new_capability` | 🔗 新增能力 | 互動式建立新的 Capability |
| `/cp.help` | ❓ 顯示說明 | 列出所有指令 |
...
Skills 是可重用的能力模組，位於 `.claude/skills/`。

### Skill 層級

```
┌─────────────────────────────────────────────────────────┐
│ Composite Skills (組合能力)                              │
│  ├─ literature-retrieval (文獻檢索 = search + filter)   │
│  └─ report-writing (讀寫報告 = read + write + validate) │
├─────────────────────────────────────────────────────────┤
│ Atomic Skills (原子技能)                                 │
│  ├─ 文獻類：literature-search, literature-filter        │
│  ├─ 報告類：pdf-reader, note-writer, content-validator │
│  ├─ 文件類：report-formatter, readme-updater, ...      │
│  ├─ Git類：git-precommit, git-pusher                   │
│  └─ 維護類：memory-updater, temp-cleaner, ...          │
└─────────────────────────────────────────────────────────┘
```
...
| 類別 | Skill ID | 說明 |
|------|----------|------|
| **文獻檢索** | `literature-search` | 多策略文獻搜尋 (MCP) |
| | `literature-filter` | 文獻過濾與確認 |
| | `literature-retrieval` | 🔗 組合：搜尋 + 過濾 |
| **報告撰寫** | `pdf-reader` | PDF 讀取與轉換 |
| | `note-writer` | 結構化筆記撰寫 |
| | `content-validator` | 內容準確性驗證 |
| | `report-formatter` | 報告格式化 |
| | `report-writing` | 🔗 組合：讀寫報告流程 |
| **文件** | `readme-updater` | README 更新 |
